...
from pickle import EMPTY_SET, FALSE
import random
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        distance = 0
        while current_place.is_hive == False and distance <= self.max_range:
            if current_place.bees != [] and self.min_range <= distance:
...
# END Problem 12
    ...
    def __init__(self, gamestate):
         # BEGIN Problem 12
         "*** YOUR CODE HERE ***"
         if gamestate.queen == 1:
             raise DuplicateQueensException()
         else:
             gamestate.queen += 1
         ScubaThrower.__init__(self, health = 1)
         # END Problem 12
    ...
    def action(self, gamestate):
        ...
        spe = self.place.exit
        while spe:
            if spe.ant is not None:
                if spe.ant.is_container and spe.ant.ant_contained is not None:
                    spe.ant.buff()
                    spe.ant.ant_contained.buff()
                else:
                    spe.ant.buff()
            spe = spe.exit
        # END Problem 12
        ...
        if self.slowed_turns > 0:
            ...
            if gamestate.time % 2 == 0:
                if self.scared_turns > 0:
                    ...
                    self.scared_turns -= 1
                    if self.place.entrance.is_hive == True:
                        if self.blocked():
